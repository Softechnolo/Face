...
def recognition_liveness(model_path, le_path, detector_folder, encodings, confidence=0.5, path_to_rec = 'uploads_for_ver/1.mp4'):
   
    args = {'model':model_path, 'le':le_path, 'detector':detector_folder, 
            'encodings':encodings, 'confidence':confidence , 'path_to_rec':path_to_rec}
    ...
    vs = FileVideoStream(path_to_rec).start()
    time.sleep(0) # wait camera to warmup
...
if __name__ == '__main__':
    name, label_name = recognition_liveness('liveness.model', 'label_encoder.pickle', 
                                            'face_detector', '../face_recognition/encoded_faces.pickle', confidence=0.5, path_to_rec= '')
