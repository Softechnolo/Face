...
## UI for the App
def run():
    st.title("RAJ Face Detection")
    activities = ["Image", "Webcam"]
    # st.set_option('deprecation.showfileUploaderEncoding', False)
    st.sidebar.markdown("# Choose Input Source")
    choice = st.sidebar.selectbox("Choose Verifictation :", activities)
    link = '[©RAJ Analytics](https://spewhub.com)'
    ...
    if choice == 'Webcam':
        st.markdown(
            '''<h4 style='text-align: left; color: #d73b5c;'>* Still working on android Camera"</h4>''',
            unsafe_allow_html=True)
